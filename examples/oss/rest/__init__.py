...
from .amp_client import AmpOssClient, AmpOssClientError
...
class AmpOssRestPlugin:
    """Minimal Hermes plugin governing tool calls through amp-community's
    self-hosted OSS REST API directly -- raw urllib, zero third-party
    dependencies. See README.md for setup, and ../sdk-python/ for the
    same governance logic built on amp-sdk-python instead."""

    def __init__(self) -> None:
        self._config = load_config()
        self._client = AmpOssClient(self._config)
        self._warned = False

    def _warn_unconfigured(self) -> None:
        if not self._warned:
            logger.warning(
                "amp-oss-rest-example is not configured -- set AMP_OSS_BASE_URL "
    ...
    def pre_tool_call(
        ...
        **_: Any,
        ...
        if not self._config.is_configured:
            ...
            return None
        try:
            result = self._client.submit_request(
                self._config.policy_id, action.tool, action.action, action.context,
            )
        except AmpOssClientError as exc:
            message = f'AMP OSS governance is unavailable; blocked "{tool_name}". Error: {exc}'
    ...
    def _await_decision(self, tool_name: str, request_id: str) -> Optional[Dict[str, str]]:
        ...
        while time.time() < deadline:
            ...
            try:
                polled = self._client.get_request(request_id)
            except AmpOssClientError as exc:
                logger.warning("amp-oss-rest-example poll failed: %s", exc)
                continue
            status = str(polled.get("status") or "")
            if status == "pending":
                continue
            # status is a generic terminal marker ("resumed"/"closed"); the
            # actual outcome is the separate "decision" field.
            decision = str(polled.get("decision") or "")
            if decision == "approved":
                return None
            if decision == "modified":
                # The original call was NOT executed -- amp-community spawns
                # a new follow-up review for the modified params instead of
                # resuming this one. Treating this as "allow" would tell
                # Hermes to proceed with params that were never approved.
                follow_up = polled.get("follow_up_hitl_id") or ""
                return self._block(
            ...
            return self._block(
                f'"{tool_name}" was {decision or "rejected"} by AMP HITL review.'
            )
        return self._block(f'"{tool_name}" timed out waiting for AMP HITL approval.')


_PLUGIN = AmpOssRestPlugin()
